...
def choice():
    ...
    if (user_list.startswith(start_list) and 
       user_list.endswith(end_list) and 
       user_list != "" and
       len(user_list) > len(start_list) + len(end_list)):
        try:
            if end_list != "":
                inner_list = user_list[len(start_list):-len(end_list)]
            else:
                inner_list = user_list[len(start_list):]
            items = [item.strip() for item in inner_list.split(separation_list) if item.strip()]
            choice_user_list = random.choice(items)
...
def shuffle():
    ...
    if (user_list.startswith(start_list) and 
        ...
        try:
            ...
            result = start_list + separation_list.join(items) + end_list
            entry_tab5.insert(END, result)
        except MemoryError:
